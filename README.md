...
<!-- 1. [Описание](#описание)
1. [Использование темплейта](#использование-темплейта)
1. [Бланки ответов](#бланки-ответов)
...
1. [Файл с задачами](#файл-с-задачами)
    ...
    1. [Листы](#листы)
    1. [Прочее](#прочее)
1. [Файл с решениями](#файл-с-решениями)
...
1. [Скачайте последнюю версию шаблона заданий, решений и бланка ответов.](https://github.com/Beyond-Curriculum/qazcho-latex/releases/)
1. На сайте overleaf.com нажмите на кнопку **New Project** и выберите опцию **Upload Project**.
1. Загрузите `.zip` файл скачанного шаблона.
...
Но перед этим небольшая ремарка. В папках [tasks](/tasks), [solutions](/solutions), [blank](/blank) есть примеры оформленных условий, решений и листов ответов. Вы можете пользоваться ими как справочным ресурсом. Можете прям копировать код задачи и начинать его модифицировать при оформлении своей задачи.

## Оформляем условие задачи

Для оформления условий пользуемся шаблоном `tasks`. В файле `main.tex` в корне есть несколько строчек, но нам важна только одна:

```latex
\input{problems/great-problem/main}
```

Этой строчкой мы говорим: возьми условие задачи из папки `problems/great-problem/main.tex`. Туда мы и идем. Все остальные файлы в корне шаблона нужны для корректной компиляции, но никакие правки в них вносить не нужно. Кстати, переименуйте папку `great-problem` в название вашей задачи (название может быть рабочим, например `antonorganics`). В самом конце вам нужно будет отправить Председателю Коллегии только содержимое этой папки.
...
## Как оформить листы ответов?

Пользуемся шаблоном `blank`.
...
Пользуемся шаблоном `sol`.

### Боксы с решением

#### Решения обычных задач
...
Если есть какая-то другая часто используемая единица измерения, которой нет в таблице - пожалуйста сообщите.
